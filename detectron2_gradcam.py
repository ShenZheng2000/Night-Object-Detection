...
class Detectron2GradCAM():
  ...
  def get_cam(self, target_instance, layer_name, grad_cam_instance):
      ...
      cam_orig : numpy.ndarray
        unprocessed raw cam
      """
      model = TwoPCTrainer.build_model(self.cfg)

      checkpoint = torch.load(self.cfg.MODEL.WEIGHTS)

      # Extract the model weights
      model_weights = checkpoint['model']
      # Filter and remap the weights (example: selecting only backbone weights)
      backbone_weights = {k.replace('modelTeacher.', ''): v 
    ...
    #   checkpointer.load(backbone_weights)
