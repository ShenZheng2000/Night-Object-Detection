...
class TwoPCTrainer(DefaultTrainer):
    def __init__(self, cfg):     # NOTE: OK, keep this
        """
        Args:
            cfg (CfgNode):
    ...
    def resume_or_load(self, resume=True):  # NOTE: OK, keep this
        """
    ...
    @classmethod
    def build_evaluator(cls, cfg, dataset_name, output_folder=None): # NOTE: OK, keep this
    ...
    @classmethod
    def build_train_loader(cls, cfg):  # NOTE: OK, keep this
        mapper = DatasetMapperTwoCropSeparate(cfg, True)
        return build_detection_semisup_train_loader_two_crops(cfg, mapper)

    @classmethod
    def build_lr_scheduler(cls, cfg, optimizer):  # NOTE: OK, keep this
        return build_lr_scheduler(cfg, optimizer)

    def train(self):  # NOTE: OK, keep this
        self.train_loop(self.start_iter, self.max_iter)
        if hasattr(self, "_last_eval_results") and comm.is_main_process():
            verify_results(self.cfg, self._last_eval_results)
            return self._last_eval_results

    def before_train_json(self): # NOTE: OK, keep this
    ...
    def train_loop(self, start_iter: int, max_iter: int): # NOTE: OK, keep this
    ...
    # ================== Pseduo-labeling ==================
    # =====================================================
    def threshold_bbox(self, proposal_bbox_inst, thres=0.7, proposal_type="roih"): # NOTE: OK, keep this
    ...
    def remove_label(self, label_data): # NOTE: OK, keep this
        for label_datum in label_data:
            if "instances" in label_datum.keys():
                del label_datum["instances"]
        return label_data

    def add_label(self, unlabled_data, label): # NOTE: OK, keep this
        for unlabel_datum, lab_inst in zip(unlabled_data, label):
            unlabel_datum["instances"] = lab_inst
        return unlabled_data
    
    def get_label(self, label_data): # NOTE: OK, keep this
        label_list = []
        for label_datum in label_data:
            if "instances" in label_datum.keys():
    ...
    def process_data(self, unlabel_data, unlabel_dep_data=None, record_dict=None, prefix=None): # TODO: write this separately
...
# NOTE: set basline teacher to empty for now
